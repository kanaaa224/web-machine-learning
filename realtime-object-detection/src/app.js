...
export default class App {
    ...
    async initialize(selectors = 'main') {
        ...
        document.querySelector(this.ownedSelectors).innerHTML = `
            <div>
                <input id="start" type="button" value="開始" disabled>
            </div>
            <div style="position: relative; aspect-ratio: 16/9; background-color: #00000040;">
                <video id="video" style="width: 100%; height: auto; display: none;"></video>
                <canvas id="canvas" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;"></canvas>
            </div>
            <p id="status">モデルデータをロード中...</p>
        `;

        this.videoContainer = document.querySelector(`${this.ownedSelectors} div[style*="position: relative"]`);

        const resize = () => {
            this.videoContainer.style.width = window.innerWidth > window.innerHeight ? '50%' : 'auto';
        ...
        this.button          = document.querySelector(`${this.ownedSelectors} #start`);
        this.button.disabled = false;
        this.button.onclick  = () => this.start();

        this.video  = document.querySelector(`${this.ownedSelectors} #video`);
        this.canvas = document.querySelector(`${this.ownedSelectors} #canvas`);
        this.ctx    = this.canvas.getContext('2d');

        this.status = document.querySelector(`${this.ownedSelectors} #status`);

        this.status.innerText = 'モデルデータのロードが完了しました';
